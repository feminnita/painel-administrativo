...
export function mapApiProduct(p: ApiProduct): AdminProduct {
  return {
    ...
    sale_start: p.saleStart ?? null,
    sale_end: p.saleEnd ?? null,
    stock: p.stock ?? 0,
    active: p.active ?? true,
    visible_in_store: p.visibleInStore ?? true,
...
export function toApiProduct(f: ProductInput): Record<string, unknown> {
  return {
    ...
    saleStart: f.sale_start,
    saleEnd: f.sale_end,
    stock: f.stock,
    active: f.active,
    visibleInStore: f.visible_in_store,
    featured: f.featured,
    isNew: f.is_new,
    isBestseller: f.is_bestseller,
    isOutlet: f.is_outlet,
    images: f.images,
